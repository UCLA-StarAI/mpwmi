...
class oldMPWMI:
    ...
    smt_solver : pysmt.Solver instance
        The SMT solver used by oldMPWMI
    tolerance : float
    ...
    def _compute_message(self, x, y, evidence=None):
        ...
        for lc, uc in zip(cpts, cpts[1:]):
            ...
            if len(x_y_potentials) > 0:
                ...
                potential_msgs = oldMPWMI._parse_potentials(
    ...
    def piecewise_symbolic_integral(self, integrand, x, y=None):
        ...
        for l, u, p in integrand:
            ...
            if self.cache is not None:  # for cache = True
                ...
                """
                bds_ks = [oldMPWMI.cache_key(syml)[0],
                          oldMPWMI.cache_key(symu)[0]]  # cache keys for bounds
                bds = [syml.as_expr(),
                       symu.as_expr()]
                p_ks = oldMPWMI.cache_key(symp)  # cache key for integrand polynomial
    ...
    def _get_msgs_intersection(self, msgs: list):
        ...
        intersection = oldMPWMI._line_sweep(points, len(msgs))
